...
for i=2:length(XvHistMaxPeaks)
	...
	plot(XhBins(1:length(Dataset.Hibaap.XhHistDerSmooth)), Dataset.ImReader.imHeight-6*Hibaap.graphSpacing-(3*XhHistDerSmooth),'b-', 'LineWidth',2);
	% 0-line of derivative
	plot(XhBins(1:length(Dataset.Hibaap.XhHistDerSmooth)), Dataset.ImReader.imHeight-6*Hibaap.graphSpacing-0,'g-', 'LineWidth',2);
	...
	% % plot signchange
	% if WindowsColVoteBin(i) == 0
	% 	id, Dataset.ImReader.imHeight-6*Hibaap.graphSpacing-D(id)
	% 	disp('plotting cross');
	% 	pause;
	% 	plot(id,Dataset.ImReader.imHeight-6*Hibaap.graphSpacing-D(id), '+g', 'LineWidth',2, 'MarkerSize', 10);
	% end
