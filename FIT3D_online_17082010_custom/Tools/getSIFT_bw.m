...
function [F,Files] = getSIFT_bw(directory,maxFrames,step, METHOD)
...
%tmpim1 = rgbImage;
        ...
        if(strcmp(METHOD,'LOWE'))
            [image, descriptors, loc] = sift([img1FileBW]);
        % VL_FEAT    
        elseif(strcmp(METHOD,'VL_FEAT'))
            %[f1,d1] = vl_sift(single(rgb2gray(tmpim1)));
            [f1,d1] = vl_sift(single(tmpim1));
			%figure;imshow(tmpim1);
